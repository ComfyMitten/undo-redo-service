...
## Prevent accessing this singleton at runtime by guarding with `Engine.is_editor_hint()`.
##
## NOTE: this service largely functions as a workaround for certain EditorUndoRedoManager & UndoRedo
## behaviors that aren't currently supported, but you could choose to instead compile the engine
...
## We need to track the time of the last UndoRedo's commit (via the history_updated signal) so we
## can know if too much time has passed for a new merge to be possible.
...
## NOTE: the default value is the same as INT64_MIN in Godot 4.7+, set manually for compatibility.
...
## Clears the undo cache once enough time has passed that we're sure no new actions can be merged.
var _merged_undo_operations_clear_timer: Timer

## Track the undo operations last committed to a merged editor action.
...
## undo or redo is currently in progress. This returns `false` at runtime.
...
## in the History tab.
## If the code is designed to run under those circumstances, then we can pass `true` here, in which
## case we'll do that `is_committing_action()` check and return `false` if there is no mid-progress
## action already, indicating that this code ran due to an undo or redo and thus is invalid.
...
## Queue a 'do' method call for an object. See EditorUndoRedoManager.add_do_method() for details.
...
## Queue a 'do' property change for a property on an object. See
## EditorUndoRedoManager.add_do_property() for details.
...
## Queue a 'do' reference to an object, allowing the object to be unreferenced or freed when the
## UndoRedo 'do' history is cleared; see EditorUndoRedoManager.add_do_reference() for details.
func queue_do_reference(object: Object) -> void:
	_queued_operations.append(_undo_redo_manager.add_do_reference.bind(object))


## Queue an 'undo' method call for an object. See
## EditorUndoRedoManager.add_undo_method() for details.
...
## Queue an 'undo' property change for a property on an object. See
## EditorUndoRedoManager.add_undo_property() for details.
...
## UndoRedo 'undo' history is cleared; see EditorUndoRedoManager.add_undo_reference() for details.
...
## Helper to queue both a do and undo method call at once for one object. The same method will be
## called each way, with the given `do_args` passed on 'do', and `undo_args` passed on 'undo'.
...
## using the queued operations from the `queue_` functions called previously, with the MERGE_ALL
## mode. This will wipe the queued operations list immediately afterward.
## If there is already an action being processed by the UndoRedoManager, this will automatically
## wait for it to finish before proceeding.
## If we detect that the action can't be merged, `standalone_action_name` will be used as its name.
## It's technically possible, but very unlikely, that we may think an action can be merged when it
## cannot, in which case a new action with the same name as the last one will be created.
##
...
##
## 1. It is acceptable to create a new action in the UndoRedo history if a merge isn't possible.
## 2. The correct, matching backward_undo_ops value is chosen for the previous action to merge into.
##    For most editor-derived actions, this will be the default of `false`.
...
##    previous action to finish processing.
...
##    inspector, we did not then add an undo operation that modifies that same property.
...
##    undoing from 2 -> 1 -> 0, or in other words the initial undo operation is 'idempotent'.
...
##    version control system (like git) and save your changes to it often.
...
## A more straightforward commit operation that can be used in place of EditorUndoRedoManager's
## `create_action()` and `commit_action()` functions, to process queued operations that don't need
...
##
## 1. A non MERGE_DISABLE mode must be chosen
## 2. There must already be an action to merge into
## 3. The last action's name must match the new one's name
## 4. The last action's 'backward_undo_ops' mode must match the new one's (default false)
## 5. The last action must have been created or updated less than 800ms ago
##
...
## can't be accounted for here since the EditorUndoRedoManager & UndoRedo APIs don't expose a way to
## check the specific parameters of an already-committed operation. So we provide an overridable
## backward_undo_ops in the commit function defaulting to false that can be set if needed.
